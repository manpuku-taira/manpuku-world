...
/* =========================================================
  PATCH 99 FINAL STABILIZE
  - グローバルアナウンス基盤を一本化
  - バトル結果の「〇〇 VS 〇〇」をプレイヤー/AIとも確実表示
  - 桜蘭の陰陽術 - 闘 - を processActivatedEffect 経由へ
  - 登場時効果（04/05/11/26/27/28）をP1側で任意発動化
  - sendCharacterToWing を最新安定版へ戻しつつ、効果除去時のみ文脈表示
========================================================= */
(function(){
  "use strict";

  /* ---------------- グローバルアナウンス ---------------- */
  function ensureFinalAnnounceUI(){
    ...
    const box = document.createElement('div');
    box.style.width = 'min(86vw, 430px)';
    box.style.background = 'rgba(10,10,16,.96)';
    box.style.border = '1px solid rgba(255,255,255,.14)';
    box.style.borderRadius = '14px';
    box.style.padding = '18px 16px 14px';
    ...
    ok.style.color = '#fff';
    ok.style.fontWeight = '800';
    ok.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); hideFinalAnnounce(); }, {passive:false});

    box.addEventListener('click', (e)=> e.stopPropagation());
    overlay.addEventListener('click', ()=> hideFinalAnnounce());
  ...
  let finalAnnQueue = [];
  let finalAnnBusy = false;

  function flushFinalAnnounce(){
    if(finalAnnBusy) return;
    const next = finalAnnQueue.shift();
    if(!next) return;
    finalAnnBusy = true;
    ensureFinalAnnounceUI();
    const overlay = document.getElementById('mwFinalAnnounceOverlay');
    const title = document.getElementById('mwFinalAnnounceTitle');
    const body = document.getElementById('mwFinalAnnounceBody');
    if(!overlay || !title || !body){ finalAnnBusy = false; return; }
    title.textContent = next.title || 'アナウンス';
    body.textContent = next.text || '';
    overlay.style.display = 'flex';
  }

  function hideFinalAnnounce(){
    const overlay = document.getElementById('mwFinalAnnounceOverlay');
    if(overlay) overlay.style.display = 'none';
    finalAnnBusy = false;
    setTimeout(flushFinalAnnounce, 0);
  }

  window.mwPushAnnounce = function(title, text){
    ensureFinalAnnounceUI();
    finalAnnQueue.push({title, text});
    flushFinalAnnounce();
  };
  ...
  /* ---------------- 無効化アナウンス ---------------- */
  const __mw99_processActivatedEffect = processActivatedEffect;
  processActivatedEffect = async function(link){
    const result = await __mw99_processActivatedEffect(link);
    if(result && result.ok === false && result.detail){
      const src = link?.sourceCard?.name || link?.label || '効果';
      let by = '無効効果';
      if(result.detail.negatorKind === 'HANDGATA') by = '手形';
      if(result.detail.negatorKind === 'MEMORY') by = '記憶抹消';
      window.mwPushAnnounce('効果が無効化されました', `${src}\nは\n${by}\nによって無効化されました。`);
    }
    return result;
  };

  /* ---------------- 効果除去の文脈保持 ---------------- */
  window.__mw99RemovalContext = null;
  if(typeof activateHandCard === 'function'){
    const __mw99_activateHandCard = activateHandCard;
    activateHandCard = async function(side, handIndex, card, targetZone, targetPos){
      if(card && /ウイングに送る/.test(card.text || '')){
        window.__mw99RemovalContext = { type:'effect', effectName: card.name };
      }else{
        window.__mw99RemovalContext = null;
      }
      return await __mw99_activateHandCard(side, handIndex, card, targetZone, targetPos);
    };
  }
  if(typeof resolveEffectCard === 'function'){
    const __mw99_resolveEffectCard = resolveEffectCard;
    resolveEffectCard = async function(side, eff){
      if(eff && /ウイングに送る/.test(eff.text || '')){
        window.__mw99RemovalContext = { type:'effect', effectName: eff.name };
      }else{
        window.__mw99RemovalContext = null;
      }
      return await __mw99_resolveEffectCard(side, eff);
    };
  }

  /* ---------------- sendCharacterToWing を最新安定版へ戻す ---------------- */
  sendCharacterToWing = async function(side, uid){
    const p = state[side];
    const pos = p.C.findIndex(c=>c && c.uid===uid);
    if(pos<0) return;
    ...
    if(card && window.__mw99RemovalContext && window.__mw99RemovalContext.type === 'effect'){
      window.mwPushAnnounce(
        '効果による除去',
        `${window.__mw99RemovalContext.effectName}の効果で\n${card.name}はウイングに送られます。`
      );
    }
    ...
    for(let i=0;i<3;i++){
      ...
      while(a && state.AI.C[i] && state.AI.C[i].uid===a.uid && a.flags.attackedCountThisTurn < getMaxAttacks('AI', a)){
        ...
        if(best.type==='C'){
          ...
          if(typeof markSevenStarHit === 'function') markSevenStarHit(a, t.uid);
          ...
          if(atkA > atkD){
            ...
            if(!savedD){
              ...
              log(`AI：撃破 ${t.name} → あなたウイング`);
              await tryCattleTrigger_P1();
              if(a.no===23 && typeof mw32HasAnyEquip === 'function' && mw32HasAnyEquip(a)){
                await breakOneShieldByEffect('P1', a.name);
              }
            }
          }else if(atkA < atkD){
  ...
  /* ---------------- 登場時効果の任意発動最終版 ---------------- */
  async function mw99RunOptionalEnterEffect(side, card, message, resolveFn){
    ...
    const act = {
      ...
      resolve: resolveFn,
      onNegated: async (r)=>{
        if(r && r.negatorKind==='MEMORY') await sendCharacterToWing(side, card.uid);
        log(`${card.name} の登場時効果は無効`);
        renderAll();
      }
    };
  ...
  const __mw99_prevOnEnter = onEnterTriggers;
  onEnterTriggers = async function(side, ctx){
    const {card, pos} = ctx || {};
    if(!card) return;
    ...
    if(card.no === 4){
      await mw99RunOptionalEnterEffect(side, card, '聖ラウスの登場時効果を発動しますか？（クランプスをサーチ）', async ()=>{
        await searchFromDeckOrWingByTag(side, 'クランプス', 1, { aiAuto: side==='AI' });
      });
      return;
    }
    ...
    if(card.no === 5){
      await mw99RunOptionalEnterEffect(side, card, '統括AI タータの登場時効果を発動しますか？（2ドロー）', async ()=>{
        draw(side, 2);
        log(`${sideName(side)}：タータ登場 → 2ドロー`);
        renderAll();
      });
    ...
    if(card.no === 11){
      ...
      await mw99RunOptionalEnterEffect(side, card, '司令の登場時効果を発動しますか？（装備化してATK+500）', async ()=>{
        if(side==='AI') await aiTryShireiEquip('AI', pos);
        else await activateShireiEquip(side, pos, card);
      });
      return;
    }

    if(card.no === 26 || card.no === 27){
      await mw99RunOptionalEnterEffect(side, card, `${card.name}の登場時効果を発動しますか？`, async ()=>{
        await resolveRubySapphireEnter(side, card, ctx);
      });
      return;
    }
    ...
    if(card.no === 28){
      await mw99RunOptionalEnterEffect(side, card, 'セシア＆アリサの登場時効果を発動しますか？（怨霊撲滅屋GBのアイテムをサーチ）', async ()=>{
        await searchDeckOrWingByTitleTagItem(side, '怨霊撲滅屋GB', 1, { aiAuto: side==='AI' });
      });
      return;
    }

    return await __mw99_prevOnEnter(side, ctx);
  };

  log('PATCH 99 FINAL STABILIZE 読み込み完了');
})();
