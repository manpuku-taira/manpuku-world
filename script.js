(function(){
  ...
  function __mw_boot(){
    ...
    /* ================================
      ...
      /* ================================
        1) ルール定数
      ================================= */
      const MAX_CARD_NO = 23; // 今回は 23〜29 を実装（21・22・30は除外）
      const HAND_LIMIT = 7;
  ...
  // --- PATCH: ops/help modal should always be closable (2026-02-25) ---
  (function attachOpsClose(){
    const txt = (el)=> (el && (el.innerText || el.textContent) || "").trim();
    const isCloseBtn = (el)=> el && el.tagName === "BUTTON" && /閉じる|Close|×/.test(txt(el));
    const hide = (modal)=>{
      if(!modal) return;
      modal.style.display = "none";
      modal.classList.remove("show","open","is-open","active");
      modal.setAttribute("aria-hidden","true");
    };
    const findModalByTitle = (title)=>{
      const nodes = Array.from(document.querySelectorAll("body *"));
      for (const n of nodes){
        if (n && n.children && n.children.length && /dialog|modal|overlay/i.test(n.className||"")){
          if (txt(n).includes(title)) return n;
        }
      }
      // fallback: any element that contains the title and has a close button
      for (const n of nodes){
        if (txt(n).includes(title)){
          const btns = Array.from(n.querySelectorAll("button"));
          if (btns.some(isCloseBtn)) return n.closest(".modal,.dialog,.overlay,[role='dialog']") || n;
        }
      }
      return null;
    };
    const modal = document.getElementById("mw_ops") || document.getElementById("opsModal") || findModalByTitle("操作");
    if(!modal) return;

    // bind close buttons inside
    const btns = Array.from(modal.querySelectorAll("button"));
    btns.forEach(b=>{
      if(isCloseBtn(b)){
        b.addEventListener("click", (e)=>{ e.preventDefault(); e.stopPropagation(); hide(modal); }, {passive:false});
      }
    });

    // tap outside to close (if modal uses overlay container)
