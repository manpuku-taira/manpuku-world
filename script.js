...
const CardRegistry = [
  ...
  /* ---------------- 追加：No.23〜30 ---------------- */
  { no:23, name:"怨霊撲滅屋レイチェル", type:"character",
    tags:["除霊","霊媒師"], titleTag:"GB-ゴーストバスターズ-",
    text: normalizeText(
      "このカードは登場できず、手札または自分ステージのキャラクターカード1枚をウイングに送り、手札から見参できる。\n" +
      "このキャラクターがアイテムを装備している時、相手はタグ「怨霊」または「霊魂」を持つカードの効果を発動できない。\n" +
      "このキャラクターがバトルに勝利した時、相手シールドを1枚破壊する。"
    ),
    rank:5, atk:2100, summon:"kensan" },

  { no:24, name:"セクシーコスチューム", type:"item",
    tags:["コスプレ","防具"], titleTag:"GB-ゴーストバスターズ-",
    text: normalizeText(
      "自分のターンに手札から発動できる。自分ステージのキャラクター1体を選択し、このカードを装備する。ATK+500。\n" +
      "タグ「除霊」を持つキャラクターが装備した場合、さらにATK+500。\n" +
      "さらに、このカードを装備しているキャラクターは1ターンに合計3回攻撃できる。"
    ),
    rank:0, atk:0 },

  { no:25, name:"バグバグ大王ドゥーン", type:"character",
    tags:["bug","ラスボス","GAME"], titleTag:"BUGBUG西遊記",
    text: normalizeText(
      "このカードは登場できず、手札または自分ステージのキャラクターカード1枚をウイングに送り、手札から見参できる。\n" +
      "このカードがバトル・効果で相手によってウイングへ送られた時、発動できる。\n" +
      "ランク4以下の「小太郎」「小次郎」を2体まで見参（空きスロットに特殊登場）させる。"
    ),
    rank:5, atk:1300, summon:"kensan" },

  { no:26, name:"魔法少女ルビー", type:"character",
    tags:["美少女戦士","妹"], titleTag:"MAGIAGIA-マギアギア-",
    text: normalizeText(
      "このカードは「魔法少女サファイア」が自分ステージに存在する時のみ見参できる。\n" +
      "このカードが登場した時、手札を1枚ウイングへ送り、デッキからタグ「美少女戦士」カードを手札に加える。\n" +
      "自分ステージに存在する限り、自分ステージのタグ「美少女戦士」キャラクターのATK+500。"
    ),
    rank:4, atk:1900, summon:"kensan" },

  { no:27, name:"魔法少女サファイア", type:"character",
    tags:["美少女戦士","姉"], titleTag:"MAGIAGIA-マギアギア-",
    text: normalizeText(
      "このカードは「魔法少女ルビー」が自分ステージに存在する時のみ見参できる。\n" +
      "このカードが登場した時、手札を1枚ウイングへ送り、デッキからタグ「美少女戦士」カードを手札に加える。\n" +
      "自分ステージに存在する限り、自分ステージのタグ「美少女戦士」キャラクターのATK+500。"
    ),
    rank:4, atk:2000, summon:"kensan" },

  { no:28, name:"セシア＆アリサ", type:"character",
    tags:["冒険者","幼馴染","美少女戦士"], titleTag:"MAGIAGIA-マギアギア-",
    text: normalizeText(
      "このカードは登場できず、手札または自分ステージのキャラクターカード1枚をウイングに送り、手札から見参できる。\n" +
      "このカードが登場した時、デッキから「怨霊撲滅屋GB」をサーチして手札へ。\n" +
      "自分ターンに1度、手札の「怨霊撲滅屋レイチェル」を条件無視で見参できる。"
    ),
    rank:5, atk:2000, summon:"kensan" },

  { no:29, name:"オオカミネコ", type:"character",
    tags:["獣人","感覚","GAME"], titleTag:"BUGBUG西遊記",
    text: normalizeText(
      "このカードは登場できず、手札または自分ステージのキャラクターカード1枚をウイングに送り、手札から見参できる。\n" +
      "自分ターンに1度、デッキ・ウイングからタイトルタグ「BUGBUG西遊記」のアイテムカードを1枚手札に加える。"
    ),
    rank:5, atk:2300, summon:"kensan" },

  // No.30：カード情報が未提示のため、最低限の「通常キャラ」として仮実装（検索対象として動く形）
  { no:30, name:"怨霊撲滅屋GB", type:"character",
    tags:["除霊"], titleTag:"GB-ゴーストバスターズ-",
    text: normalizeText(
      "（仮実装）\n" +
      "※No.30の正式なカードテキストがあれば、この欄を正規実装に差し替えます。"
    ),
    rank:4, atk:1800 },
];
...
function ensureInitialCollectionAndDeck(){
  ...
  if(!Array.isArray(deck) || deck.length!==40){
    ...
    log("デッキ編集：初期デッキ（No.01〜20を各2枚）を作成");
  }
...
const state = {
  ...
  limits: {
    P1: { handgataUsed:false, cruellaUsed:false, tataUsed:false, wolfcatUsed:false, cessiaUsed:false },
    AI: { handgataUsed:false, cruellaUsed:false, tataUsed:false, wolfcatUsed:false, cessiaUsed:false },
  },
...
function resetPerTurn(side){
  ...
  state.limits[side].wolfcatUsed = false;
  state.limits[side].cessiaUsed = false;
...
function hasRachelLockAgainst(sideTryingToActivate){
  const opp = opponent(sideTryingToActivate);
  // No.23が装備中なら封印
  return state[opp].C.some(c=>{
    if(!c || c.no!==23) return false;
    if(!c.equipUid) return false;
    return !!findEquipInE(opp, c.equipUid);
  });
}
...
function calcCurrentAtk(side, card){
  ...
  // ルビー/サファイア：美少女戦士+500（両方同じ効果）
  const hasRuby = hasOnStage(side, (c)=>c && c.no===26);
  const hasSapp = hasOnStage(side, (c)=>c && c.no===27);
  if((hasRuby || hasSapp) && card.tags && card.tags.includes("美少女戦士")){
    atk += 500;
  }
...
function maxAttacksPerTurn(side, card){
  if(!card || !isCharacter(card)) return 0;
  let maxAtk = 1;
  if(card.no===7 && card.equipUid) maxAtk += 1; // まひる：装備中2回
  if(card.equipUid){
    const eq = findEquipInE(side, card.equipUid);
    if(eq && eq._addAttacks) maxAtk += (eq._addAttacks||0);
  }
  return maxAtk;
}
...
function canActivateFromViewer(card, ctx){
  ...
  // レイチェル装備封印（怨霊/霊魂）
  if(hasRachelLockAgainst(side)){
    if(card.tags && (card.tags.includes("怨霊") || card.tags.includes("霊魂"))){
      return {ok:false, reason:"レイチェル（装備中）：怨霊/霊魂の効果は発動できません"};
    }
  }
  ...
  if(side==="P1"){
    ...
    if([1,3,5,6,9,10,11,13,28,29].includes(card.no)) return {ok:true, reason:""};
...
function isAttackableNow_P1(card){
  ...
  const maxAtk = maxAttacksPerTurn("P1", card);
...
async function onClickYourC(pos){
  ...
  if(card.summon==="kensan"){
    await doKensanSummon("P1", pos, state.selectedHandIndex, {ignoreRubySapp:false});
    return;
  }
...
function kensanConditionOk(side, card, opt={}){
  if(!card || card.summon!=="kensan") return true;

  // 26/27：相互条件（ignore指定があれば無視）
  if(!opt.ignoreRubySapp){
    if(card.no===26){
      return hasOnStage(side, (c)=>c && c.no===27);
    }
    if(card.no===27){
      return hasOnStage(side, (c)=>c && c.no===26);
    }
  }
  return true;
}

async function doKensanSummon(side, cPos, handIdx, opt={}){
  const p = state[side];
  ...
  if(!kensanConditionOk(side, card, opt)){
    if(card.no===26) log("ルビー：サファイアが自分ステージにいないため見参できません", "warn");
    if(card.no===27) log("サファイア：ルビーが自分ステージにいないため見参できません", "warn");
    return;
  }
...
async function equipItemFromE(side, ePos, itemCard){
  ...
  if(itemCard.no===18){
    ...
    // セクシーコスチューム
    itemCard._equipBonus = 500;
    if(host.tags.includes("除霊")) itemCard._equipBonus2 = 500;
    itemCard._addAttacks = 2; // 合計3回攻撃（通常1回 +2）
  }
...
async function onEnterTriggers(side, ctx){
  ...
  // 26/27：登場時 手札1→ウイング、デッキから美少女戦士サーチ
  if(card.no===26 || card.no===27){
    if(side==="AI"){
      // AIは簡易：手札があれば1枚捨ててサーチ
      const p = state[side];
      if(p.hand.length){
        const moved = p.hand.shift();
        moveToWing(side, moved);
        log(`AI：${card.name} 登場→手札1枚ウイング（${moved.name}）`);
      }
      await searchFromDeckByTag(side, "美少女戦士", 1, {aiAuto:true});
      renderAll();
      return;
    }
    ...
    const p = state[side];
    if(!p.hand.length){
      log(`${card.name}：手札がないためコストを払えず不発`, "warn");
      return;
    }
    const ok = await askYesNo("効果確認", `${card.name}の効果を使用しますか？（手札1枚→ウイング / 美少女戦士サーチ）`);
    if(!ok) return;

    // 手札1枚コスト
    const items = p.hand.map((c,i)=>({
      label:`手札：${c.name}`, sub:`No.${pad2(c.no)}`, value:String(i), card:c
    }));
    const pick = await askChoice(card.name, "ウイングへ送る手札を1枚選んでください。", items);
    const idx = Number(pick);
    const moved = p.hand.splice(idx,1)[0];
    moveToWing(side, moved);
    log(`${card.name}：コスト→ウイング ${moved.name}`);

    await searchFromDeckByTag(side, "美少女戦士", 1);
    renderAll();
    return;
  }

  // 28：登場時「怨霊撲滅屋GB」サーチ（デッキ）
  if(card.no===28){
    if(side==="AI"){
      await searchFromDeckByNameIncludes(side, "怨霊撲滅屋GB", 1, {aiAuto:true});
      return;
    }
    if(await askYesNo("効果確認", "セシア＆アリサの効果を使用しますか？（怨霊撲滅屋GBをサーチ）")){
      await searchFromDeckByNameIncludes(side, "怨霊撲滅屋GB", 1);
    }
    return;
  }
}
...
async function activateFieldCardAbility(side, zone, pos, card){
  ...
  // 28：自ターン1度 レイチェル条件無視で見参（手札から）
  if(card.no===28){ await activateCessiaSummonRachel(side); return; }

  // 29：自ターン1度 BUGBUGアイテムサーチ
  if(card.no===29){ await activateWolfcatSearch(side); return; }
...
async function activateCessiaSummonRachel(side){
  if(state.limits[side].cessiaUsed){
    log("セシア＆アリサ：このターンは既に使用しています", "warn");
    return;
  }
  const p = state[side];
  const idx = p.hand.findIndex(c=>c && c.no===23);
  if(idx<0){
    log("セシア＆アリサ：手札にレイチェルがいません", "warn");
    return;
  }
  const empty = findEmptyIndex(p.C);
  if(empty<0){
    log("セシア＆アリサ：C枠が空いていません", "warn");
    return;
  }

  const ok = await askYesNo("セシア＆アリサ", "手札のレイチェルを条件無視で見参しますか？（コストは必要です）");
  if(!ok) return;

  state.limits[side].cessiaUsed = true;

  // レイチェルを選択した状態にして、見参を実行
  state.selectedHandIndex = idx;
  renderAll();
  await doKensanSummon(side, empty, idx, {ignoreRubySapp:true});
  state.selectedHandIndex = null;
  renderAll();
}

async function activateWolfcatSearch(side){
  if(state.limits[side].wolfcatUsed){
    log("オオカミネコ：このターンは既に使用しています", "warn");
    return;
  }
  if(!(await askYesNo("オオカミネコ", "BUGBUG西遊記のアイテムをサーチしますか？"))){
    return;
  }
  state.limits[side].wolfcatUsed = true;
  await searchBugbugItemFromDeckOrWing(side, 1);
  renderAll();
}
...
async function canActivateEffectNow(side, eff){
  // レイチェル装備封印（怨霊/霊魂）
  if(hasRachelLockAgainst(side)){
    if(eff.tags && (eff.tags.includes("怨霊") || eff.tags.includes("霊魂"))){
      return false;
    }
  }
...
async function resolveEffect(side, eff){
  ...
  if(eff.no===2){
    if(side==="AI"){
      ...
      if(mode==="MAX"){
        await sendCharacterToWing(enemy, best.uid, {reason:"effect", by:side});
        log(`AI：フレイムバレット（MAX）→ ${best.name} をウイングへ`);
        return;
      }else{
        for(const c of r4){
          await sendCharacterToWing(enemy, c.uid, {reason:"effect", by:side});
        }
...
async function searchFromDeckByTag(side, tag, n, opt={}){
  const p = state[side];
  const pool = p.deck.filter(c=>c && c.tags && c.tags.includes(tag));
  if(!pool.length){ log(`サーチ失敗：デッキにタグ「${tag}」が見つかりません`, "warn"); return; }
  for(let k=0;k<n;k++){
    if(opt.aiAuto){
      const moved = p.deck.splice(p.deck.findIndex(c=>c && c.tags.includes(tag)), 1)[0];
      if(moved) p.hand.push(moved);
      log(`AI：サーチ（${tag}）`);
      continue;
    }
    const items = pool.map(c=>({
      label:`${c.name}`, sub:`DECK / TAG:${tag}`, value:c.uid, card:c
    }));
    const pick = await askChoice("サーチ", `デッキからタグ「${tag}」を手札へ（${k+1}/${n}）`, items);
    const moved = removeFromZone(p.deck, String(pick));
    if(moved) p.hand.push(moved);
  }
  log(`サーチ：デッキからタグ「${tag}」を手札へ`);
}

async function searchFromDeckByNameIncludes(side, word, n, opt={}){
  const p = state[side];
  const pool = p.deck.filter(c=>c && c.name && c.name.includes(word));
  if(!pool.length){ log(`サーチ失敗：デッキに「${word}」が見つかりません`, "warn"); return; }
  for(let k=0;k<n;k++){
    if(opt.aiAuto){
      const idx = p.deck.findIndex(c=>c && c.name.includes(word));
      if(idx>=0){
    ...
    const items = pool.map(c=>({
      label:`${c.name}`, sub:`DECK / NAME:${word}`, value:c.uid, card:c
    }));
    const pick = await askChoice("サーチ", `デッキから「${word}」を手札へ（${k+1}/${n}）`, items);
    const moved = removeFromZone(p.deck, String(pick));
    if(moved) p.hand.push(moved);
  }
  log(`サーチ：デッキから「${word}」を手札へ`);
}

async function searchBugbugItemFromDeckOrWing(side, n){
  const p = state[side];
  const pool = [
    ...p.deck.map(c=>({src:"deck", c})),
    ...p.wing.map(c=>({src:"wing", c})),
  ].filter(x=>x.c && x.c.titleTag==="BUGBUG西遊記" && x.c.type==="item");

  if(!pool.length){ log("BUGBUGサーチ：対象のアイテムが見つかりません", "warn"); return; }

  for(let k=0;k<n;k++){
    const items = pool.map(x=>({
      label:x.c.name,
      sub:`${x.src.toUpperCase()} / BUGBUG / ITEM`,
      value:`${x.src}:${x.c.uid}`,
      card:x.c
    }));
    const pick = await askChoice("BUGBUGサーチ", `BUGBUG西遊記のアイテムを手札へ（${k+1}/${n}）`, items);
    const [src, uid] = String(pick).split(":");
    if(src==="deck"){
      const moved = removeFromZone(p.deck, uid);
      if(moved) p.hand.push(moved);
...
async function destroyOneShieldToHand(defSide){
  ...
  log(`追加効果：${sideName(defSide)} シールド${idx+1} → 手札へ`);
  return true;
}
...
async function resolveBattle(attacker, defenderUid){
  ...
  if(atkA > atkD){
    await sendCharacterToWing("AI", defender.uid, {reason:"battle", by:"P1"});
    log(`撃破：${defender.name} → AIウイング`);

    // No.23：勝利時 相手シールド1枚破壊（装備中）
    if(attacker.no===23 && attacker.equipUid){
      const eq = findEquipInE("P1", attacker.equipUid);
      if(eq){
        await destroyOneShieldToHand("AI");
      }
    }
    ...
    if(!saved){
      await sendCharacterToWing("P1", attacker.uid, {reason:"battle", by:"AI"});
    ...
    if(!savedD) await sendCharacterToWing("AI", defender.uid, {reason:"battle", by:"P1"});
    log("相打ち：双方ウイング");
  }
...
async function tryDuunSummonTwins(ownerSide){
  // No.25：相手によってウイングへ送られた時、ランク4以下の「小太郎」「小次郎」を2体まで見参
  const p = state[ownerSide];
  const emptySlots = [];
  for(let i=0;i<3;i++) if(!p.C[i]) emptySlots.push(i);
  if(!emptySlots.length){
    log("ドゥーン：空きCがないため不発", "warn");
    return;
  }
  ...
  const pool = [
    ...
    if(!x.c || !isCharacter(x.c)) return false;
    if((x.c.rank||0) > 4) return false;
    return (x.c.name.includes("小太郎") || x.c.name.includes("小次郎"));
  });

  if(!pool.length){
    log("ドゥーン：対象（小太郎/小次郎）が見つからず不発", "warn");
    return;
  }

  const max = Math.min(2, emptySlots.length);
  for(let k=0;k<max;k++){
    const items = pool.map(x=>({
      label:x.c.name,
      sub:`${x.src.toUpperCase()} / R${x.c.rank||0}`,
      value:`${x.src}:${x.c.uid}`,
      card:x.c
    })).concat([{label:"終了", value:"X"}]);

    const pick = await askChoice("ドゥーン", `小太郎/小次郎を見参（${k+1}/${max}）`, items);
    if(pick==="X") break;

    const [src, uid] = String(pick).split(":");
    let moved=null;
    if(src==="hand") moved = removeFromZone(p.hand, uid);
    if(src==="deck") moved = removeFromZone(p.deck, uid);
    if(src==="wing") moved = removeFromZone(p.wing, uid);
    if(!moved) continue;

    const pos = emptySlots.shift();
    p.C[pos]=moved;
    log(`ドゥーン：特殊登場（見参） ${moved.name}`);
    renderAll();
    await onEnterTriggers(ownerSide, {zone:"C", pos, card:moved});
    await sleep(80);
  }
}

async function sendCharacterToWing(side, uid, opt={}){
  const p = state[side];
  ...
  // No.25：相手によって送られた時（battle/effect いずれも）
  if(card.no===25){
    const by = opt.by || null; // "P1"/"AI"
    const isByOpponent = (by && by !== side);
    if(isByOpponent){
      log("ドゥーン：相手によってウイングへ送られた→効果発動");
      await tryDuunSummonTwins(side);
    }
  }
}
...
function itemBonusForHost(item, host){
  ...
  if(item.no===18){
    ...
    b = 500 + (host.tags.includes("除霊") ? 500 : 0);
    b += 280; // 追加攻撃ぶんの評価
  }
  return b;
}
...
async function aiTryPlayBestItem(){
  ...
  if(item.no===18){
    ...
    item._equipBonus = 500;
    if(host.tags.includes("除霊")) item._equipBonus2 = 500;
    item._addAttacks = 2;
  }
...
async function aiBattleBest(){
  ...
  for(let i=0;i<3;i++){
    ...
    const maxAtk = maxAttacksPerTurn("AI", a);
    if(a.flags.attackedCountThisTurn>=maxAtk) continue;
    ...
    if(best.type==="C"){
      ...
      if(atkA > atkD){
        await sendCharacterToWing("P1", t.uid, {reason:"battle", by:"AI"});
        log(`AI：撃破 ${t.name} → あなたウイング`);
        await tryCattleTrigger_P1();

        // AI側レイチェル勝利時（装備中）も同様
        if(a.no===23 && a.equipUid){
          const eq = findEquipInE("AI", a.equipUid);
          if(eq){
            await destroyOneShieldToHand("P1");
          }
        }
        ...
        if(!saved){
          await sendCharacterToWing("AI", a.uid, {reason:"battle", by:"P1"});
        ...
        if(!savedA) await sendCharacterToWing("AI", a.uid, {reason:"battle", by:"P1"});
        if(!savedD){
          await sendCharacterToWing("P1", t.uid, {reason:"battle", by:"AI"});
          await tryCattleTrigger_P1();
        }
...
async function init(){
  ...
  log(`初期：所持=各3枚 / デッキ=No.01〜20を各2枚（40固定）`);
  log("追加：No.23〜30 実装");
}
