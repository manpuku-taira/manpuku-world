const ASSET_FIELD = "assets/field.png";
...
const CARD_IMAGE_FILES = [
  ...
  "11_司令.png",
  "12_班目プロデューサー.png",
  "13_超弩級砲塔列車スタマックス氏.png",
  "14_記憶抹消.png",
  "15_桜蘭の陰陽術闘.png",
  "16_力こそパワー.png",
  "17_キャトルミューティレーション.png",
  "18_axブラスター01放射型.png",
  "19_聖剣アロンダイト.png",
  "20_普通の棒.png",
];
...
function cardImagePathByNo(no){
  const file = CARD_IMAGE_FILES[no-1];
  return `assets/cards/${file}`;
}

function setFieldBackground(){
  el.fieldBg.style.backgroundImage = `url(${ASSET_FIELD})`;
}
...
function openCardViewer(card){
  ...
  el.viewerImg.src = cardImagePathByNo(card.no);
...
function openZoneList(side, zoneKey){
  ...
  if(list.length===0){
    ...
    list.forEach((c)=>{
      ...
      th.style.backgroundImage = `url(${cardImagePathByNo(c.no)})`;
...
function renderStage(side, root){
  ...
  for(let i=0;i<3;i++){
    ...
    if(c){
      ...
      face.style.backgroundImage = `url(${cardImagePathByNo(c.no)})`;
...
function startGame(){
  ...
  setFieldBackground();
  renderAll();
  log("画像が出ない場合：VercelのURL末尾に ?v=99 を付けて開き直してください", "muted");
}
